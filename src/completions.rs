...
mod parse {
    ...
    where
        ...
        E: ParseError<I>,
    {
        SeparatedList0LastRaw { parser: f, separator: sep }
    }
    ...
    impl<I, E: ParseError<I>, F, G> Parser<I> for SeparatedList0LastRaw<F, G>
    where
        I: Clone + Input,
        F: Parser<I, Error = E>,
        G: Parser<I, Error = E>,
    {
        type Output = (Vec<<F as Parser<I>>::Output>, I);
        ...
        fn process<OM: OutputMode>(
            ...
            use nom::{Check, Err, Mode, OutputM};

            let mut res = OM::Output::bind(|| (nom::lib::std::vec::Vec::new(), i.clone()));
            ...
            loop {
                let len = i.input_len();
                match self.separator.process::<OutputM<Check, Check, OM::Incomplete>>(i.clone()) {
                    Err(Err::Error(_)) => return Ok((i, res)),
                    Err(Err::Failure(e)) => return Err(Err::Failure(e)),
                    Err(Err::Incomplete(e)) => return Err(Err::Incomplete(e)),
                    Ok((i1, _)) => {
...
/// Tab completion for `:knock`
fn complete_iamb_knock(args: Vec<String>, store: &ChatStore) -> Vec<String> {
    match args.len() {
        1 => complete_choices(&args[0], &["send", "accept", "reject", "ban"]),
        2 if args[0] == "send" => {
            // Don't complete with the known room aliases since the users most likely wants to knock
            // on an unknown room.
            vec![]
        },
        2 if ["accept", "reject", "ban"].contains(&args[0].as_str()) => {
            complete_users(&args[1], store)
        },
        _ => vec![],
    }
}
...
fn complete_iamb_verify(args: Vec<String>, store: &ChatStore) -> Vec<String> {
    ...
    match args.len() {
        ...
        2 if subcmds.contains(&args[0].as_str()) && args[0] != "recover" => {
            complete_verification(&args[1], store)
        },
        _ => vec![],
    }
}
...
fn complete_iamb_room(args: Vec<String>, store: &ChatStore) -> Vec<String> {
    ...
    if args.len() == 1 {
        ...
        match args[0].as_str() {
            ...
            "version" => complete_choices(input, &["show", "upgrade"]),
...
fn complete_cmdarg(
    ...
    *cursor = new_cursor;
    ...
    let mut completions = match cmd.name.as_str() {
        ...
        "knock" => complete_iamb_knock(args, store),
